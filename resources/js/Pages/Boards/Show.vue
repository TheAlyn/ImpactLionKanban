...
function onCardDrop(evt, column) {
  const card = evt.item.__underlying_vm__
    ...
    <DialogModal :show="showCardModal" @close="closeCardModal">
      ...
      <template #content>
        <div class="space-y-4">
          ...
          <div class="grid grid-cols-2 gap-4">
            <input
              v-model="cardForm.color"
              type="color"
              class="w-full h-10 p-0 border-0 rounded cursor-pointer"
            />
            <input
          ...
          <input
            v-model="cardForm.background_image"
            type="text"
      ...
      <template #footer>
        <button
          ...
          class="px-4 py-2 bg-gray-700 text-white rounded hover:bg-gray-600 mr-2"
        >Cancelar</button>
        <button
          @click="submitCard"
    ...
    <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8 text-white">
      ...
      <header class="mb-6 flex items-center justify-between">
        ...
        <Link
          ...
          class="bg-gray-700 px-4 py-2 rounded hover:bg-gray-600 transition"
        >Voltar</Link>
      </header>
        ...
        <template v-if="board.columns?.length">
          ...
          >
              ...
              <Dropdown align="right" :contentClasses="['py-1','bg-gray-800','text-gray-200']">
                ...
                <template #content>
                  <ul class="divide-y divide-gray-700">
                    ...
                    <li><button class="block w-full text-left px-4 py-2 hover:bg-gray-700">📤 Mover todos os cartões</button></li>
                    <li><button class="block w-full text-left px-4 py-2 hover:bg-gray-700">👀 Seguir</button></li>
                    <li><button class="block w-full text-left px-4 py-2 hover:bg-gray-700">🎨 Alterar cor (Upgrade)</button></li>
                    <li><button class="block w-full text-left px-4 py-2 hover:bg-gray-700">🗄️ Arquivar Lista</button></li>
                    <li><button class="block w-full text-left px-4 py-2 hover:bg-gray-700">🗑️ Excluir Lista</button></li>
                    <li><button class="block w-full text-left px-4 py-2 hover:bg-gray-700">📦 Arquivar todos os cartões</button></li>
                  </ul>
                </template>
              </Dropdown>
            </header>
            ...
            <Draggable
              ...
              @end="evt => onCardDrop(evt, column)"
              class="flex-grow overflow-y-auto space-y-3"
            >
              <template #item="{ element: card }">
                <div class="bg-gray-700 rounded-md p-3 shadow cursor-pointer hover:bg-gray-600 transition">
          ...
          <template v-else>
            <div class="w-full bg-gray-900 rounded-md p-4">
              ...
              <div class="flex space-x-2">
                <button
                  @click="submitCreateColumn"
                  class="bg-blue-600 text-white px-3 py-1 rounded hover:bg-blue-700 transition"
                >Adicionar Lista</button>
                <button
                  @click="cancelCreateColumn"
                  class="text-white text-xl font-bold px-2"
                  title="Cancelar"
                >✕</button>
              </div>
            </div>
